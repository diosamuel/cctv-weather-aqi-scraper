...
CHART_SQL_PATH = BASE_DIR / "schema.sql"
...
def load_camera_rows(con: duckdb.DuckDBPyConnection) -> List[tuple]:
    """Select camera_code and route_slug from cctv_list, stripping '/' from route_slug."""
    rows = con.execute(
        """
...
def normalize_camera_api_id(camera_row: tuple) -> str:
...
def ingest_hourly_vehicle_speed(con: duckdb.DuckDBPyConnection, camera_code: str, payload: Dict[str, Any]):
    sql = """
...
def crawl_once(con: duckdb.DuckDBPyConnection, camera_rows: List[tuple]):
    for row in camera_rows:
        ...
        try:
            ...
            ingest_vehicle_speed(con, camera_code, c1)
            ingest_hourly_vehicle_speed(con, camera_code, c2)
            ingest_timeseries_vehicle_speed(con, camera_code, c3)

"""
Fetch BMKG weather forecast for each CCTV in cctv_list_final (by adm4),
then insert into bmkg_weather table.

API: GET https://api.bmkg.go.id/publik/prakiraan-cuaca?adm4={adm4}
...
DUCKDB_PATH = BASE_DIR / os.environ.get("SOURCE_DUCKDB", "cctv.duckdb")
...
def fetchBmkgWeather(adm4: str) -> dict | None:
    """
...
def main() -> None:
    ...
    rows = con.execute(
        """
        SELECT DISTINCT adm4
        FROM cctv_list_final
        WHERE adm4 IS NOT NULL
        ORDER BY adm4
        """
    ).fetchall()
    ...
    for i, (adm4,) in enumerate(rows):
        ...
        inserted += 1
        print(
            f"INSERT adm4={adm4!r}  "
            f"{lokasi.get('provinsi')}, {lokasi.get('kotkab')}, {lokasi.get('kecamatan')}, {lokasi.get('desa')}"
        )

    con.close()
    print(f"\nDone. {inserted} inserted, {errored} errors, {len(rows)} total adm4 codes.")
